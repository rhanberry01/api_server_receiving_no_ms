    ...
    */
    ...
    async add_receiving_ms(srspos, user_id, receiving_counter, po_no, inv_no, inv_amount, po_order, supplier_id, remarks) {
        ...
        let data = {
            ...
            address: po_order.delivery_address,
            contactperson: '',
            EDA: delivery_date,
            ...
            deliveryaddress: po_order.delivery_address,
            createdby: user_id,
    ...
    async fetch_po_inquiry(rr_no, date_from, date_to){
        ...
        let row = await Db.connection('srspos')
                          .select('description', 'remarks', 'receivingno')
                          .from('receiving')
    ...
    async update_po(trans_id, trans_type) {
        ...
        if(refs.length == 0) {
            ...
            throw new CustomException({ message:  'NO RESULT FOUND IN REFS' }, 401)
        }    
        ...
        if(purch_order.length == 0) {
            return 'NO RESULT FOUND IN PURCH ORDER'
        }
        ...
        await Promise.all( _.each(purch_order_details, async (row) => {
